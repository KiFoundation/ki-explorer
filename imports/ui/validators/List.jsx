...
const ValidatorRow = (props) => {
    ...
    return (
        <SentryBoundary>
            <Row className="validator-info bg-white my-2 py-3 list-border block-info block-info-custom cursor-pointer"  onClick={() => props.history?.push("/validator/"+props.validator.operator_address)}>
                <Col className="d-none d-md-block counter data" xs={2} md={1}>{props.index+1}</Col>
                <Col xs={12} md={2} className="data"><Link to={"/validator/"+props.validator.operator_address}><Avatar moniker={moniker} profileUrl={props.validator.profile_url} address={props.validator.address} list={true} /><span className="moniker font-800 primary-color">{moniker}</span></Link></Col>
                <Col className="voting-power data text-left" xs={{size:8, offset:2}} md={{size:3, offset:0}} lg={2}><i className="material-icons d-md-none">power</i>  <span>{props.validator.voting_power?numbro(props.validator.voting_power).format('0,0'):0} ({props.validator.voting_power?numbro(props.validator.voting_power/props.totalPower).format('0.00%'):"0.00%"})</span></Col>
                ...
                {(!props.inactive)?
                    <Col style={{background: '#eee', borderRadius: 3}} className="uptime data pr-0" xs={{size:2,order:"last"}} md={2}>
                        {
                            props.validator.uptime &&
                            <Progress barClassName="custom-uptime" bar value={props.validator.uptime}>
                            ...
                            ||
                            <span className="text-center w-100 d-block">0</span>
                        }
                    </Col>:''}
