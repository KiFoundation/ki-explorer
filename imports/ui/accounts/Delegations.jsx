    ...
    render(){
        return (
            <Container fluid>
                ...
                {this.props.delegations.sort((b, a) => (a.balance.amount - b.balance.amount)).map((d, i) => {
                    return <Row key={i} className="delegation-info bg-white my-2 py-3 list-border">
                          <Col xs={6} md={4} className="text-nowrap overflow-auto primary-color font-800"><AccountTooltip address={d.delegation.validator_address} /></Col>
                        <Col xs={3} md={4}>{numbro(d.delegation.shares).format("0,0")}</Col>
                        <Col xs={3} md={4}>{new Coin(d.balance.amount).stakeString()}</Col>
                    </Row>
                })}
            </Container>
        );
